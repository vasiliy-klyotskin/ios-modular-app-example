...
extension DemoUtilsFeature {
    public static func make(model: DemoUtilsModel) -> DemoUtilsFeature {
        ...
        let rootEvents = DemoUtilsMainEvents(onRequestsSelection: { weakFlow.obj?.goToRequests(model: $0) })
        let requestsEvents = DemoUtilsRequestsEvents(onRequestSelected: { weakFlow.obj?.goToEditRequest(model: $0) })
